...
**- visual/data/hospitales_madrid_nodos.csv**: esto es un csv con todos los nombres de los hospitales de la ciudad de Madrid, su latitud, su longitud y el número del nodo (cruce de calles) más cercano al hospital en el grafo madrid_grafo.

**- visual/data/madrid_grafo.rar**: esto es un archivo comprimido (descomprimelo) con un grafo con todas las calles de la ciudad. Las aristas son calles y las intersecciones nodos. La librería OSMnx lo lee y lo pasa a un objeto MultiDiGraph nativo de la librería NetworkX.
...
Descomprime la carpeta en sí y dentro el archivo visual/data/madrid_grafo.rar. Asegúrate de que tu terminal está situada en la carpeta donde has descargado estos archivos. (comando cd ruta)
...
```bash
python -m pip install -r visual/legacy/requirements.txt
```
**3. Corre el código**
Ejecuta este comando, te abrirá una pestaña en el navegador:
```bash
python -m streamlit run visual/legacy/prototipo_ambulancIA.py
```
